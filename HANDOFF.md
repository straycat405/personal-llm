...
> **⚠️ 최우선 과제가 바뀌었다.** 2026-08-27 후반 작업에서 요약 청크 가설을 검증했으나 효과가
> 없었고, 그 진단 과정에서 **컨텍스트 초과**라는 더 근본적인 병목을 찾았다. 아래 1번을 먼저 한다.
> 상세 경위는 `docs/portfolio.md` 7장과 개선 로그 참고.

1. **컨텍스트 예산 관리 — 현재 최우선**

   실측: 본문 청크 평균 **5,425자**(최대 10,117자) → 청크당 약 1,507토큰 →
   **topK=3 근거만 약 4,521토큰**인데 `num_ctx`는 **4,096**이다.
   시스템 프롬프트·질문·도구 스키마를 넣기도 전에 이미 초과다. 검색이 찾아준 근거를 모델이
   끝까지 보지도 못하고 있다.

   비교할 후보(같은 골든셋 8문항으로 판정):
   - (a) 청크 크기 축소 — `TokenTextSplitter` 1500 → 800. 임베딩 시간 증가와 트레이드오프
     (과거 실측: 6.5초 → 31.4초, 대신 Recall 100%)
   - (b) `num_ctx` 4096 → 8192 — KV 캐시 증가로 VRAM 압박. 8GB에서 위험도 측정 필요
   - (c) 도구 결과 길이 상한 — 청크별로 잘라 topK는 유지하되 총량을 제한

   **주의**: 현재 평가 코퍼스는 38청크·206,147자다(이전 21청크·31,096자에서 바뀜).
   기존 기준선과 비교하지 말 것. 현재 코퍼스 기준선은 **0.0% 통과 / 사실 38.1% / 평균 65.8s**다.

2. **요약 청크 기능 재평가** — `DocumentSummarizer`는 동작하고 개요형 질의에서 1위로 검색되지만,
   현재 코퍼스에서는 본문 `1 사업 개요` 청크와 중복이라 점수가 오르지 않았다. 컨텍스트 문제를
   해결한 뒤 기본 활성 여부를 다시 판단한다. (기능 자체는 유지 중)
