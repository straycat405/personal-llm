...
`./gradlew providerComparisonExperiment` 최신 실측
(청크 800 코퍼스·동일 8문항·동일 검색 파이프라인·모델만 교체):
...
**문항별 성공·실패 패턴이 두 모델에서 완전히 일치한다.**

| 문항 | 두 모델 공통 | 의미 |
|---|---|---|
| overview-1, round-general-1, round-local-1, eligibility-1, application-1 | ✅ 성공 | 회귀 감시용 |
| **track-1, registration-1, prize-1** | ❌ 실패 | **모델 교체로 안 풀림** — 파이프라인·데이터 문제 |

→ 앞으로 검색·청킹 개선의 효과는 **이 실패 3문항으로만 판정**하면 된다.
...
3. **provider 비교 반복 실행** — 재비교는 완료했고 **로컬이 상용과 동률**이 나왔다
   (아래 참고). 다만 각 1회 실행이라 분산 미측정이다. 로컬은 같은 8문항에서 0%~62.5%로
   흔들린 전력이 있어, 결론을 굳히려면 3회 반복이 필요하다.
