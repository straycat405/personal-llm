...
로컬 LLM의 응답 품질과 체감 성능을 함께 개선한다.

**이번 세션 흐름**: 사용자가 PDF 구현을 잠시 중단시키고 **보안·성능·안정성·유지보수
보수작업을 먼저 하라**고 지시 → P0 4건 완료 → P1 #5(GPU admission control) 완료 →
사용자가 "지금 상태로 써도 되는지" 물어 서버를 직접 띄워 실사용 테스트 진행 → 그 과정에서
나온 버그·기능요청 4건(새 대화 버튼, 모델 목록 최신화, 업로드 진행률, 드래그 앤 드롭)
수정 → **사용자가 여기서 보수작업을 중단하고 포트폴리오 제작으로 넘어가기로 결정**.
`docs/portfolio-improvement-log.md`에 이번 세션 전체(P0 4건·P1 #5·사용자 테스트 버그 4건)를
문제→설계결정→구현→검증→한계 형식으로 기록해뒀다 — 포트폴리오 작성 시 원재료로 그대로 쓸 것.

**현재 지시**: P1 #6~8과 P2는 **의도적으로 보류된 백로그**다(아래 표) — 로컬 단일 사용자
앱이라는 현재 threat model 대비 한계효용이 낮다는 판단으로 사용자가 직접 결정했다. 다음
세션은 기본적으로 **포트폴리오 제작을 돕는 것**이 목적이다. 사용자가 명시적으로 "보수
계속하자"고 하지 않는 한 P1 #6 이후로 먼저 진입하지 말 것.
...
6. ⏸️ **보류(사용자 결정)** — P1 #6: ETL bounded executor/파서 예산. 착수 시 결정 필요: ETL 전용 별도 큐 vs P1 #5의 `OllamaGenerationQueue` 공유
7. ⏸️ **보류** — P1 #7: provider/model·비용 통제 (서버 allowlist, cache 상한, 외부 provider 토큰·동시성 한도)
8. ⏸️ **보류** — P1 #8: SSE/progress 수명 관리 (raw thread 제거, owner 검증, TTL cleanup, subscriber 제한) — 현재 permitAll인 이유는 P0 #3 완료 기록의 "의도적으로 안 건드린 것" 참고
9. ⏸️ **보류** — P2 유지보수: 스트리밍 문자열 누적, 검색 쿼리·페이지네이션, DB 마이그레이션·운영 프로필
10. ⏸️ **보류** — 미커밋 PDF 파서 실험 재개 (표 구조 실험, 아래 6절 회귀기준 5개 미검증 상태로 대기)

**사용자 테스트에서 나온 수정 4건 (P0/P1과 별개, 완료)**: 사이드바 "+" 새 대화 버튼 무반응
수정, LLM 모델 목록 최신화(Claude/Gemini/OpenAI 전부 구형이었음), ETL 업로드 진행률 실제
퍼센티지로 전환(기존 6단계 임의 점프), RAG 업로드 모달 드래그 앤 드롭 지원. 자세한 내용은
`docs/portfolio-improvement-log.md`의 "서버를 직접 띄워보고 나온 사용성 갭 4건" 참고.

**저장소 상태**:

- 브랜치: `main`
- `HEAD` = `origin/main` = `9bc16e4`
- 작업 트리는 **깨끗하지 않다.** PDF 표 구조 보존 실험과 평가 하네스 변경이 미커밋 상태다
  (이번 세션 내내 그대로 보존됨 — 여러 차례 `git stash`로 분리해가며 보수 커밋과 섞이지
  않게 했다).
- 이 미커밋 변경은 사용자 작업으로 간주한다. 덮어쓰기·되돌리기·삭제·혼합 커밋 금지.
